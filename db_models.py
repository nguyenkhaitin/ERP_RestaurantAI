from sqlalchemy import create_engine, Column, Integer, String, DateTime, Float, Date, JSON, Boolean
from sqlalchemy.orm import declarative_base, sessionmaker
from sqlalchemy.sql import func
import datetime

# --- CẤU HÌNH KẾT NỐI (SỬA PASSWORD NẾU CẦN) ---
# ⚠️ THAY ĐỔI PASSWORD CỦA BẠN TRONG URL NÀY
DB_URL = "postgresql://postgres:123@localhost:5433/postgres"
...
# 4. Bảng Trạng thái Bàn (AI Update trực tiếp)
class TrangThaiBan(Base):
    __tablename__ = "trang_thai_ban"
    id = Column(Integer, primary_key=True, index=True)
    ban_id = Column(Integer, unique=True, nullable=False, index=True)  # ID bàn
    so_khach_ai = Column(Integer, default=0)                           # Số khách AI đếm được
    canh_bao = Column(Boolean, default=False)                          # Có cảnh báo không
    loai_canh_bao = Column(String(50), nullable=True)                  # "KHACH_AO", "QUEN_DONG", "NGOI_LAU"
    lan_cap_nhat_ai_cuoi = Column(DateTime, default=datetime.datetime.now)  # Lần cập nhật cuối
